...
public class AzureMapService : IRequestHandler<LocationInfo>
{
    private readonly ILogger<AzureMapService> _logger;
    private readonly HttpClient _httpClient;
    private readonly AzureMapConfiguration _azureMapConfig;
    ...
    public AzureMapService(
        ...
        AzureMapConfiguration azureMapConfiguration,
        HttpClient httpClient,
        OtelMetrics metrics)
    {
        _logger = logger;
        _httpClient = httpClient;
        _azureMapConfig = azureMapConfiguration;
        _metrics = metrics;
    }
    ...
    public async Task<LocationInfo> ProcessAsync(WatchRequest? watchRequest)
    {
        ...
        finally
        {
            ...
            _metrics.SetProcessingDuration(
                processTimer.ElapsedMilliseconds, sourceKind, locationInfo.RequestStatus.StatusCode, "AzureMap");
            _metrics.IncreaseProcessedCounter(sourceKind, locationInfo.RequestStatus.StatusCode, "AzureMap");
        }
    }
    ...
    public async Task<LocationInfo> RequestLocationName(decimal lat, decimal lon)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            _logger.LogWarning(response.StatusCode == HttpStatusCode.Unauthorized
                ? $"Unauthorized access to AzureMap"
        ...
        string locality;
        var region = "";
        var address = resource.GetProperty("address");
