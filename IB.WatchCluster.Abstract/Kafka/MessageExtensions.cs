...
namespace IB.WatchCluster.Abstract.Kafka
{
    public static class MessageExtensions
    {
        public static MessageHeader GetHeader(this Message<string, string> message)
        {
            ...
            if (messageType == null)
                throw new InvalidOperationException($"Can not parse message of unknown type. Type {rawMessageType} not found");
        ...
        public static bool TryParseMessageValue(this Message<string, string> rawMessage, Type messageType, out object messageValue)
        {
            messageValue = JsonSerializer.Deserialize(rawMessage.Value, messageType);
            if (messageValue == null)
                Log.Warning("Can not parse message of type {@messageType}, raw message {@rawMessage}", messageType, rawMessage.Value);
